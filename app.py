...
# Function to extract stock symbol and exchange


def extract_stock_info(stock_info_str):
    try:
        stock_info = json.loads(stock_info_str)
        if isinstance(stock_info, dict) and 'symbol' in stock_info:
            return stock_info.get('symbol'), stock_info.get('exchange')
        else:
            return stock_info_str, None  # Fallback to treat string as symbol
...
if submitted:
    ...
    with st.spinner('Converting company name to stock symbol...'):
        stock_info_str = main.get_stock_symbol(company_input)
        stock_symbol, exchange = extract_stock_info(stock_info_str)
    ...
    def load_stock_data(symbol, start, end, exchange=None):
        try:
            logger.info(
            ...
            if exchange:
                ...
                logger.info(f"Using ticker '{ticker}' for yfinance download.")
            else:
                ticker = symbol
    ...
    with st.spinner('Fetching market data...'):
        stock_data = load_stock_data(stock_symbol, start_date, end_date, exchange)
